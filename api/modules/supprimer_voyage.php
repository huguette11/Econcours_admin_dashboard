<?php
    ...
    else 
    {
        if(isset($_GET['id_voyage'])) 
        {
            ...
            $id = mysqli_real_escape_string($db,htmlspecialchars($_GET['id_voyage']));

            $requete = $bdd->prepare('UPDATE voyage SET suppression = ? WHERE id_voyage=?');
            $requete->execute(array("Oui",$id));
            
            //Historique des action
            $nom_table="voyage";
            $nom_action="Suppression voyage";
            $adresse_ip = $_SERVER['REMOTE_ADDR'];

            $requete2 = $bdd->prepare('INSERT INTO historique_action(id_voyage,adresse_ip,id_user,nom_table,nom_action) VALUES(?,?,?,?,?)');
